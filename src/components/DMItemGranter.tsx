...
export function DMItemGranter({ isOpen, onClose }: DMItemGranterProps) {
  ...
  const handleGrantToSelected = () => {
    if (selectedCharacters.length === 0) {
      setSuccessMessage('Please select at least one character')
      setTimeout(() => setSuccessMessage(''), 3000)
      return
    }

    if (!itemName.trim()) {
      setSuccessMessage('Please enter an item name')
      setTimeout(() => setSuccessMessage(''), 3000)
      return
    }
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-900 border border-dnd-gold rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className="sticky top-0 bg-gray-900 border-b border-dnd-gold p-6 flex justify-between items-center">
          <div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-4 py-2 bg-gray-800 border border-gray-600 rounded-lg text-white
                       focus:outline-none focus:ring-2 focus:ring-dnd-gold resize-none"
            />
          </div>
                ...
                {characters.map((character) => (
                  ...
                  >
                    <input
                      ...
                      className="w-5 h-5 rounded border-gray-600 text-dnd-gold
                               focus:ring-2 focus:ring-dnd-gold focus:ring-offset-gray-900"
                    />
          ...
          <div className="flex gap-4 pt-4 border-t border-gray-700">
            <button
              ...
              className={`flex-1 px-6 py-3 rounded-lg font-semibold transition-all duration-200
                         ${selectedCharacters.length > 0 && itemName.trim()
            ...
            <button
              ...
              className={`flex-1 px-6 py-3 rounded-lg font-semibold transition-all duration-200
                         ${characters.length > 0 && itemName.trim()
